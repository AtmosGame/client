...
export const SearchUserModule: React.FC = () => {
  ...
  return (
    <div className="relative h-screen bg-gray-900 text-white py-24 lg:py-36 overflow-hidden ">
      <div className="mx-auto max-w-[50%]">
        ...
        <div className="mb-10 flex justify-between items-center gap-4 z-50">
            <Controller
              name="username"
              control={control}
              render={({ field }) => <Input {...field} placeholder="Masukkan username" onChange={(e) => {
                field.onChange(e);
        ...
        {
          showCard && data.length == 0 && <div className="text-l font-bold text-gray-900 truncate dark:text-white"> Username tidak ditemukan </div>
        }
      ...
      <div className="w-full max-w-md px-4 bg-white border border-gray-800 rounded-lg shadow m:p-8 dark:bg-white-800 dark:border-gray-700 py-1 space-y-2 my-2 z-50 active:bg-white-600 transition-all hover:shadow-lg hover:shadow-white-400/25 "> <Link href = {`/viewprofile/${user.username}`}>
        <div className="flow-root  z-50">
            <ul role="list" className="divide-y divide-gray-200 dark:divide-gray-700  z-50">
                ...
                <div key={id}>  {user.username} {user.active == false && <div className="text-l font-bold text-gray-900 truncate dark:text-red" > Blocked </div>} </div>
